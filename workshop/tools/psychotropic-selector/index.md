...
<head>
  ...
  <style>
    /* ─── Tokens (matches Working Diagnosis) ────────────────────────────── */
    :root {
      --paper:      #F5F1E8;
      --paper-warm: #EDE7D8;
      --ink:        #1A1F1C;
    ...
    * { box-sizing: border-box; }
    ...
    tbody tr:hover {
      background: rgba(47, 107, 92, 0.02);
    }
    ...
    .col-dose, .col-onset, .col-halflife, .col-cost {
      font-family: var(--mono);
      font-size: 0.9rem;
      color: var(--ink-soft);
    }
    ...
    @media (max-width: 768px) {
      ...
      .col-se { max-width: 150px; }
    }

    @media print {
      .masthead, .filters, .site-footer { display: none; }
...
<body>
  ...
  <main class="wrap">
    ...
    <div class="filters">
      <div class="filter-group">
        ...
        <div class="filter-options" id="indication-filter">
          <!-- Generated by JS -->
        </div>
      </div>
      <div class="filter-group">
        <div class="filter-label">Drug Class</div>
        <div class="filter-options" id="class-filter">
          <!-- Generated by JS -->
        </div>
      </div>
      <div class="filter-group">
        <div class="filter-label">Patient Factor</div>
        <div class="filter-options" id="factor-filter">
          <!-- Generated by JS -->
        </div>
      </div>
    </div>
    ...
    <div id="results-container">
      ...
      <div class="table-wrapper">
        <table id="drugs-table">
          <thead>
            <tr>
              <th>Drug (NZF name / Brand)</th>
              <th>Class</th>
              <th>Indication(s)</th>
              <th>Usual Dose</th>
              <th>Onset</th>
              <th>Half-life</th>
              <th>Main Side Effects</th>
              <th>CYP450 Role</th>
              <th>NZ Cost</th>
              <th>Pregnancy</th>
            </tr>
  ...
  <footer class="site-footer">
    <div class="wrap">
      <p><strong>Working Diagnosis</strong> · Aotearoa New Zealand · © 2026</p>
  ...
  <script>
    ...
    const drugs = [
      ...
      },
      {
        ...
        onset: '2–4 weeks',
        halfLife: '35 hours',
        sideEffects: 'Sexual dysfunction, GI upset, tremor, QT prolongation (↑dose), hyponatraemia',
      ...
      },
      {
        ...
        onset: '3–5 weeks',
        halfLife: '21 hours',
        sideEffects: 'Insomnia, activation, dry mouth, seizure risk (↑dose/factors), weight loss',
      ...
      },
      {
        id: 'quetiapine',
        nzf: 'Quetiapine',
        brand: 'Seroquel, Asenapin, generics',
    ...
    function buildFilters() {
      const indicationContainer = document.getElementById('indication-filter');
      indications.forEach(ind => {
        indicationContainer.innerHTML += `
          <div class="filter-option">
            <input type="checkbox" id="ind-${ind}" value="${ind}" data-filter="indication">
            <label for="ind-${ind}">${ind.charAt(0).toUpperCase() + ind.slice(1)}</label>
          </div>
        `;
      });

      const classContainer = document.getElementById('class-filter');
      classes.forEach(cls => {
        classContainer.innerHTML += `
          <div class="filter-option">
            <input type="checkbox" id="cls-${cls}" value="${cls}" data-filter="class">
            <label for="cls-${cls}">${cls}</label>
          </div>
        `;
      });

      const factorContainer = document.getElementById('factor-filter');
      factors.forEach(fac => {
        factorContainer.innerHTML += `
          <div class="filter-option">
            <input type="checkbox" id="fac-${fac}" value="${fac}" data-filter="factor">
            <label for="fac-${fac}">${factorLabels[fac]}</label>
          </div>
        `;
      });
    }
    ...
    function getDrugFactors(drug) {
      const factors = [];
      if (['B2', 'B3'].includes(drug.pregnancy)) factors.push('pregnancy-risk');
      if (!drug.sideEffects.includes('weight')) factors.push('weight-neutral');
      if (!drug.sideEffects.toLowerCase().includes('anticholinergic')) factors.push('anticholinergic-risk');
      if (!drug.cyp450.includes('inhibitor')) factors.push('cyp-interactions');
      if (!['TCA', 'Antipsychotic'].includes(drug.class)) factors.push('renal-impairment', 'hepatic-impairment');
      if (!drug.class.includes('Antipsychotic') && !drug.class.includes('Benzodiazepine')) factors.push('elderly');
      return factors;
    }

    function filterDrugs() {
      const selectedIndications = Array.from(document.querySelectorAll('input[data-filter="indication"]:checked')).map(el => el.value);
      const selectedClasses = Array.from(document.querySelectorAll('input[data-filter="class"]:checked')).map(el => el.value);
      const selectedFactors = Array.from(document.querySelectorAll('input[data-filter="factor"]:checked')).map(el => el.value);

      const filtered = drugs.filter(drug => {
        const indMatch = selectedIndications.length === 0 || selectedIndications.some(ind => drug.indications.includes(ind));
        const classMatch = selectedClasses.length === 0 || selectedClasses.includes(drug.class);
        const factorMatch = selectedFactors.length === 0 || selectedFactors.every(fac => getDrugFactors(drug).includes(fac));
        return indMatch && classMatch && factorMatch;
      });
    ...
    function renderTable(filtered) {
      ...
      filtered.forEach(drug => {
        const pregnancyClass = drug.pregnancy.charAt(0).toLowerCase();
        ...
        row.innerHTML = `
          ...
          <td class="col-onset">${drug.onset}</td>
          <td class="col-halflife">${drug.halfLife}</td>
          <td class="col-se">${drug.sideEffects}</td>
          <td class="col-cyp">${drug.cyp450}</td>
          <td class="col-cost">${drug.cost}</td>
          <td class="col-pregnancy"><span class="badge-pregnancy cat-${pregnancyClass}">${drug.pregnancy}</span></td>
        `;
