---
title: Perinatal Mental Health
description: Depression, anxiety, and psychosis across pregnancy and the postnatal year - EPDS screening, treatment safety, and postpartum emergencies.
layout: topic.njk
specialty: Mental Health
---

<p class="topic-trail"><a href="/clinician-tools/">Library</a><span class="topic-trail-sep">·</span><a href="/mental-health/">Mental Health</a><span class="topic-trail-sep">·</span><span class="here">Perinatal</span></p>

<div class="panic-strip">
  <div class="panic-tile dont-miss">
    <p class="panic-tile__label">Don't miss</p>
    <ul>
      <li><strong>Postpartum psychosis</strong> - days after delivery, confusion, mood lability, perceptual change. Same-day, every time</li>
      <li>Maternal suicidality - leading cause of maternal mortality when indirect deaths are included</li>
      <li>Acute mania in pregnancy - urgent specialist input</li>
      <li>EPDS 10-12 with concern - significant distress is often hidden here</li>
    </ul>
  </div>
  <div class="panic-tile first-line">
    <p class="panic-tile__label">First-line</p>
    <dl>
      <dt>Screen</dt>
      <dd>EPDS at booking and 6-week postnatal check; offer at every contact, not only scheduled</dd>
      <dt>Mild-moderate</dt>
      <dd>CBT or IPT first-line; perinatal-aware therapist where possible</dd>
      <dt>Moderate-severe / pre-existing illness</dt>
      <dd>Sertraline has the most safety data; treat the illness</dd>
      <dt>Partner</dt>
      <dd>Screen too - paternal perinatal depression is real</dd>
    </dl>
  </div>
  <div class="panic-tile refer">
    <p class="panic-tile__label">Refer when</p>
    <ul>
      <li>Postpartum psychosis - same-day, every time</li>
      <li>EPDS ≥13 with significant impairment or risk</li>
      <li>Bipolar history pre-pregnancy or in this pregnancy</li>
      <li>Treatment-resistant illness or breastfeeding-medication uncertainty</li>
    </ul>
  </div>
  <div class="panic-tile tell">
    <p class="panic-tile__label">Tell the patient</p>
    <ul>
      <li>Untreated illness has its own risks for you and the pēpi</li>
      <li>Treatment is safer than people are often told - including breastfeeding</li>
      <li>This is illness, not failing as a parent</li>
      <li>Crisis: 1737, Lifeline 0800 543 354, or maternal mental-health team</li>
    </ul>
  </div>
</div>

<p>Perinatal mental health problems - spanning pregnancy through the first postnatal year - affect up to 20% of women and around 10% of partners.<sup>1</sup> They are the leading cause of maternal mortality in high-income countries when indirect causes are included.<sup>2</sup> They are also the most commonly missed.</p>

<div class="tier-2">
  <p class="tier-label">❍ Screening and assessment</p>
  <p>The Edinburgh Postnatal Depression Scale (EPDS) is the validated screening tool for the perinatal period. A score of at least 13 warrants clinical assessment. The EPDS is recommended at the booking visit and 6-week postnatal check in NZ, though implementation is inconsistent.<sup>3</sup> A score in the 10-12 range warrants a follow-up conversation even if it does not meet threshold - this is often where people are carrying significant distress that they have not disclosed.</p>
  <p>Partners matter. Paternal perinatal depression is underscreened and undertreated; offer an EPDS or PHQ-9 to partners at the same contact if they are present. Also screen at each contact rather than only at scheduled timepoints - onset across the perinatal period is not confined to the first postnatal weeks.</p>
  <p>Ask about suicidal ideation directly. Maternal suicide is a real risk in the perinatal period and is systematically underestimated by clinicians - do not let the context of new motherhood lower your threshold for direct enquiry.</p>
</div>

<div class="tier-2">
  <p class="tier-label">❍ Treatment and prescribing safety</p>
  <p>Psychological therapies (CBT, IPT) are first-line for mild-to-moderate perinatal depression and anxiety. For moderate-to-severe illness, or where the woman has a pre-existing psychiatric condition, pharmacotherapy is often appropriate and necessary. The benefit-risk calculation is frequently misframed: untreated severe depression in pregnancy carries its own fetal and maternal risks - including preterm birth, low birthweight, poor ant
