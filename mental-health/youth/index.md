---
title: Youth & Adolescent Mental Health
description: Anxiety, depression, self-harm, and eating disorders in young people - consultation approach, screening tools, and NZ prescribing context.
layout: topic.njk
specialty: Mental Health
---

<p class="topic-trail"><a href="/library/">Library</a><span class="topic-trail-sep">·</span><a href="/mental-health/">Mental Health</a><span class="topic-trail-sep">·</span><span class="here">Youth</span></p>

<div class="panic-strip">
  <div class="panic-tile dont-miss">
    <p class="panic-tile__label">Don't miss</p>
    <ul>
      <li>Self-harm or suicidality - ask directly, asking does not increase risk</li>
      <li>Eating disorder with medical instability - bradycardia, electrolytes, severe underweight</li>
      <li>First-episode psychosis - urgent EIP referral</li>
      <li>Indirect presentations: school refusal, somatic complaints, irritability not low mood</li>
    </ul>
  </div>
  <div class="panic-tile first-line">
    <p class="panic-tile__label">First-line</p>
    <dl>
      <dt>Consultation</dt>
      <dd>See the young person alone briefly; explain confidentiality limits at the start</dd>
      <dt>Tools (12+)</dt>
      <dd>PHQ-A and GAD-7</dd>
      <dt>If SSRI needed</dt>
      <dd>Fluoxetine - the only SSRI with robust RCT evidence in adolescents. Monitor closely first 4 weeks</dd>
      <dt>Engagement</dt>
      <dd>HEEADSSS, family involvement where safe, school as ally</dd>
    </dl>
  </div>
  <div class="panic-tile refer">
    <p class="panic-tile__label">Refer when</p>
    <ul>
      <li>Any suspected eating disorder - do not wait for "sick enough"</li>
      <li>Severe self-harm or suicidality</li>
      <li>First-episode psychosis</li>
      <li>Treatment-resistant or moderate-severe presentation</li>
    </ul>
  </div>
  <div class="panic-tile tell">
    <p class="panic-tile__label">Tell whanau</p>
    <ul>
      <li>Asking about self-harm matters and does not create risk</li>
      <li>Confidentiality has clear limits - clinician will tell you about safety</li>
      <li>Treatment works; waiting is the worst option</li>
      <li><strong>Youthline 0800 376 633</strong>; <strong>1737</strong> - free text or call</li>
    </ul>
  </div>
</div>

<p>Youth mental health is a genuine crisis in NZ by any measure. Rates of anxiety, depression, self-harm, and eating disorders among young people have risen markedly over the past decade.<sup>1</sup> Waitlists for secondary CAMHS are long. General practice is, by default, a frontline service.</p>

<div class="tier-2">
  <p class="tier-label">❍ Consultation approach and screening</p>
  <p>Adolescent presentations are often indirect: school refusal, somatic complaints (headaches, abdominal pain), irritability rather than low mood, or a caregiver presenting on behalf of a young person who has not consented to be there. Seeing the young person alone - briefly, with clear confidentiality limits explained - is best practice and builds the therapeutic relationship that makes future help-seeking more likely.</p>
  <p>The PHQ-A (adolescent version) and GAD-7 are appropriate from age 12+. For self-harm, ask directly - the evidence is clear that asking does not increase risk and often provides relief.<sup>2</sup> Frame it matter-of-factly: "Some young people who feel this way hurt themselves - has that happened for you?" A positive response opens the clinical conversation; the question itself does not create the risk.</p>
  <p>Establish confidentiality boundaries clearly at the start: what is kept confidential, and what is not (immediate safety risk to self or others). Young people are more likely to disclose if they understand and trust this boundary.</p>
</div>

<div class="tier-2">
  <p class="tier-label">❍ Medication in adolescents and eating disorders</p>
  <p>Fluoxetine is the only SSRI with robust RCT evidence in adolescent depression (NNT approximately 4, NNH for suicidality approximately 143 - benefit outweighs risk for moderate-to-severe illness).<sup>3</sup> The FDA black-box warning on antidepressants and suicidality in under-25s reflects increased suicidal ideation (not completed suicide) in short-term trials; do not let it discourage appropriate prescribing in severe illness - but monitor closely in the first 4 weeks after initiation. Sertraline is an acceptable alternative when fluoxetine is not tolerated.</p>
  <p>For eating disorders: early referral to a specialist service is time-critical. Medical instability (bradycardia, electrolyte abnormalities, severe underweight) and low BMI carry significant mortality risk and warrant prompt paediatric assessment alongside psyc
